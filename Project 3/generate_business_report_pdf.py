"""
Generate a one-page BUSINESS-FOCUSED PDF report for Steps 1-3 of the clinical pipeline.
"""
import os
import sys
import traceback
...
OUTPUT_PATH = 'Pipeline_Business_Report.pdf'
...
class BizPDF(FPDF):
    def header(self):
        ...
        self.set_x(10)
        self.cell(0, 6, 'Business Summary  |  Data Readiness Update  |  2 September 2026', new_x='LMARGIN', new_y='NEXT')
        self.ln(6)
...
def build_report():
    ...
    pdf.multi_cell(184, 4.8,
        ...
        "hospital readmissions. After cleaning and enriching the data, we retained a high-quality analytical "
        "base of 972,551 patients ready for predictive modeling. Roughly 1 in 7 patients (15%) in this "
        "population was readmitted within 30 days - representing a significant opportunity for early "
        "intervention and cost avoidance.")
    pdf.ln(2)
    ...
    metrics = [
        ...
        ('972,551', 'Clean, analysis-ready patient records'),
        ('15.0%', 'Patients readmitted within 30 days'),
        ('7', 'New clinical risk indicators created'),
    ]
    col_w = 46
    pdf.set_x(13)
    y_start = pdf.get_y()
    ...
    pdf.bullet('removed unusable and duplicate records, corrected invalid entries, filled in gaps using '
               'statistically sound methods, and standardized inconsistent labels (e.g., gender values).',
               bold_lead='Step 2 - Cleaned the data:')
    ...
    pdf.bullet('A clean, de-duplicated dataset of ~972K patients means predictive models trained next will be '
               'more accurate and trustworthy for clinical decision-making.')
    pdf.ln(1)

    # ---- Data readiness / caveats ----
    pdf.band('Data Quality Watch-Outs', color=RED)
    pdf.bullet('Gender field still contains inconsistent entries in source systems; recommend a data-entry '
               'standardization fix at the point of capture.')
    pdf.bullet('Drug name variety is very high, which expanded the dataset width; we recommend grouping drugs '
               'into therapeutic classes before final modeling for better performance and interpretability.')
    pdf.ln(2)
